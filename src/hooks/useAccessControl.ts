...
export function useAccessControl() {
  ...
  const getAdUnlockCount = (): number => {
    const todayKey = `ad_unlock_${new Date().toISOString().split('T')[0]}`
    return parseInt(localStorage.getItem(todayKey) || '0', 10)
  }

  const unlockByAd = async (): Promise<boolean> => {
    return new Promise((resolve) => {
      showRewardedAd({
        onReward: () => {
          const todayKey = `ad_unlock_${new Date().toISOString().split('T')[0]}`
          const count = getAdUnlockCount() + 1
  ...
  const getTotalLimit = (): number => {
    ...
    return FREE_DAILY_LIMIT + shareCount * SHARE_UNLOCK_PER_SHARE + getAdUnlockCount() * SHARE_UNLOCK_PER_SHARE
  }
  ...
  return { canViewFullInterpretation, getAccessStatus, getTodayViewedCount, getTotalLimit, shareCount, FREE_DAILY_LIMIT, SHARE_UNLOCK_PER_SHARE, unlockByAd, getAdUnlockCount }
}
